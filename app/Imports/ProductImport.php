...
class ProductImport implements ToCollection, WithStartRow
{
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($imported_data as $key => $value) {
            ...
            // Grupo de precio de venta tarjeta
            ...
            $card_group_price = trim($value[31]);
            if (!empty($card_group_price)) {
                $product_array['card_group_price'] = $card_group_price;
            } else {
                $product_array['card_group_price'] = '';
            }

            // Grupo de precio de venta transferencia
            ...
            $transfer_group_price = trim($value[33]);
            if (!empty($transfer_group_price)) {
                $product_array['transfer_group_price'] = $transfer_group_price;
            } else {
                $product_array['transfer_group_price'] = '';
            }

            //Custom fields. Se cambia del rango 30-33 al 34-37.
            if (isset($value[34])) {
                $product_array['product_custom_field1'] = trim($value[34]);
            } else {
                $product_array['product_custom_field1'] = '';
            }
            if (isset($value[35])) {
                $product_array['product_custom_field2'] = trim($value[35]);
            } else {
                $product_array['product_custom_field2'] = '';
            }
            if (isset($value[36])) {
                $product_array['product_custom_field3'] = trim($value[36]);
            } else {
                $product_array['product_custom_field3'] = '';
            }
            if (isset($value[37])) {
                $product_array['product_custom_field4'] = trim($value[37]);
            } else {
            ...
            if ($product_array['type'] == 'single') {
                ...
                $product_prices = $this->calculateVariationPrices(
                    floatval($dpp_exc_tax),
                    floatval($dpp_inc_tax),
                    floatval($selling_price),
                    floatval($tax_amount),
                    $tax_type,
                    floatval($profit_margin)
                );
        ...
        if (!empty($formated_data)) {
            foreach ($formated_data as $index => $product_data) {
                ...
                // Check de grupos de ventas
                if (!empty($product_data['card_group']) && !empty($product_data['card_group_price'])) {
                    $variation_card_group = $product_data['card_group'];
                    $variation_card_price = $product_data['card_group_price'];
                }

                if (!empty($product_data['transfer_group']) && !empty($product_data['transfer_group_price'])) {
                    $variation_transfer_group = $product_data['transfer_group'];
                    $variation_transfer_price = $product_data['transfer_group_price'];
                }

                // eliminar los campos de grupos de ventas del array para crear el producto
                unset($product_data['card_group']);
    ...
    private function relateProductsWithSuppliers($product, $supplierCode, $listPrice, $finalPrice)
    {
        $supplier =  Contact::where('business_id', request()->session()->get('user.business_id'))
