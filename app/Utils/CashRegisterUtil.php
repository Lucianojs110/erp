...
class CashRegisterUtil extends Util
{
    ...
    public function addSellPayments($transaction, $payments)
    {
        ...
        foreach ($payments as $payment) {
            if ($payment['method'] == 'cash') {
                ...
                if ($payment['guarani'] == null) {
                    $guarani = '0.00';
                } else {
                    $guarani = $payment['guarani'];
                }
                if ($payment['reales'] == null) {
                    $reales = '0.00';
                } else {
                    $reales = $payment['reales'];
                }
                if ($payment['euro'] == null) {
                    $euro = '0.00';
                } else {
                    $euro = $payment['euro'];
                }
                if ($payment['dolar'] == null) {
                    $dolar = '0.00';
                } else {
                    $dolar = $payment['dolar'];
                }
     ...
     * @return boolean
     */
    public function refundSell($transaction, $payments)
    {
        ...
        foreach ($payments as $payment) {
                ...
                $payments_formatted[] = new CashRegisterTransaction([
                    ...
                    'amount' => $payment['amount'] ?? 0.00, // usar 0.00 si $payment['amount'] es null
                    'pay_method' => $payment['method'],
                    'type' => 'credit',
    ...
    public function getRegisterDetails($register_id = null)
    {
        ...
        $register_details = $query->select(
            ...
            'cash_registers.closed_at as close_time',
            ...
            DB::raw("SUM(IF(pay_method='cash', IF(transaction_type='sell', guarani, 0), 0)) as guarani"),
            DB::raw("SUM(IF(pay_method='cash', IF(transaction_type='sell', reales, 0), 0)) as reales"),
            DB::raw("SUM(IF(pay_method='cash', IF(transaction_type='sell', dolar, 0), 0)) as dolar"),
            DB::raw("SUM(IF(pay_method='cash', IF(transaction_type='sell', euro, 0), 0)) as euro"),
            ...
            DB::raw("SUM(IF(transaction_type='expense', amount, 0)) as total_expense"),
