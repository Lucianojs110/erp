...
class ProductUtil extends Util
{
     ...
     * @return boolean
     */
    public function createSingleProductVariation($product, $sku, $purchase_price, $dpp_inc_tax, $profit_percent, $selling_price, $selling_price_inc_tax, $precioMayorista, $cantidadMayorista)
    {
        ...
        $variation_data = [
                ...
                'precioMayorista' => $this->num_uf($precioMayorista),
                'cantidadMayorista' => $this->num_uf($cantidadMayorista),
            ];
            ...
            if (!empty($value['variations'])) {
                ...
                foreach ($value['variations'] as $k => $v) {
                    ...
                    $variation_data[] = [
                      ...
                      'cantidadMayorista' => $this->num_uf($v['cantidadMayorista']),
                      'precioMayorista' => $this->num_uf($v['precioMayorista'])
                    ];
                    $c++;
                }
    ...
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true)
    {
        ...
        if (empty($vld)) {
            ...
            if (empty($vld)) {
                $product_id = Variation::find($variation_id)->product_id;
            }

            
            $newvld = new VariationLocationDetails();
            $newvld->variation_id = $variation_id;
            $newvld->location_id = $location_id;
            $newvld->product_id = empty($vld) ? $product_id  : $vld->product_id;
            $newvld->product_variation_id = empty($vld) ? ProductVariation::where('product_id', $product_id)->first() : $vld->product_variation_id;
            $newvld->qty_available = 0;
            $newvld->save();
        }
        ...
        if ($check_qty) {
            /* Eliminado la condición de stock para que se pueda vender sin stock
        ...
        if (!empty($location_id)) {
            ...
            // eliminada esta query para que se pueda vender sin stock
            /* $query->where(function ($query) use ($location_id) {
        ...
        $products = $query->select(
            ...
            'p.hasMayorista as hasMayorista',
