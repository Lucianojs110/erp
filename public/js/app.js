$(document).ready(function() {
    ...
    //Withholdings table
    var withholdings_table = $('#withholdings_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '/withholdings',
        columnDefs: [
    ...
    $('.withholdings_modal').on('shown.bs.modal', function() {
        $('.withholdings_modal')
            .find('.select2')
            .each(function() {
                __select2($(this));
    ...
    $(document).on('submit', 'form#withholdings_add_form', function(e) {
        e.preventDefault();
        $(this)
            .find('button[type="submit"]')
            .attr('disabled', true);
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success == true) {
                    $('div.withholdings_modal').modal('hide');
                    toastr.success(result.msg);
                    withholdings_table.ajax.reload();
                } else {
                    toastr.error(result.msg);
                }
            },
    ...
    $(document).on('click', 'button.edit_withholdings_button', function() {
        $('div.withholdings_modal').load($(this).data('href'), function() {
            $(this).modal('show');

            $('form#withholdings_edit_form').submit(function(e) {
                e.preventDefault();
                $(this)
                    .find('button[type="submit"]')
                    .attr('disabled', true);
    ...
    $(document).on('submit', 'form#withholdings_edit_form', function(e) {
        e.preventDefault();
        $(this)
            .find('button[type="submit"]')
            .attr('disabled', true);
    ...
    $(document).on('click', 'button.delete_withholdings_button', function() {
        swal({
            title: LANG.sure,
    ...
    $(document).on('click', 'a.print-delivery-receipt', function(e) {
        e.preventDefault();
        var href = $(this).data('href');
    ...
    // temporal stock table

    var agent_temporal_stock_table = $('#agent_temporal_stock_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '/agent-temporal-stock',
        columnDefs: [
            {
                targets: 3,
                orderable: false,
    ...
    // agent_stock_returns_table
    var agent_stock_returns_table = $('#agent_stock_returns_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '/agent-stock-returns',
        columnDefs: [
            {
                targets: 2,
                orderable: false,
    ...
    // temporal_stock_modal_button click
    $(document).on('click', 'button.temporal_stock_modal_button', function() {
        $('div.temporal_stock_modal').load($(this).data('href'), function() {
            $(this).modal('show');      
        });
    });

    $(document).on('click', 'button.remove_stock_row', function() {
        $(this)
            .closest('tr')
            .remove();

            // check if there are still any rows and disable submit button if not
            if ($('#temporal_stock_table tbody tr').length == 0) {
                $('button[type="submit"]').attr('disabled', true);

                // close modal
                $('div.temporal_stock_modal').modal('hide');
            }
    });
    ...
    $(document).on('click', 'button.add_delivery_button', function() {
        $('div.delivery_modal').load($(this).data('href'), function() {
            $(this).modal('show');
        });
    }); 
    ...
    // delivery agent commission set percentage when selected an option
    $(document).on('change', 'select[name=user_id]', function() {
        var user_id = $(this).val();
        var commission = $(this).find('option:selected').data('commission');
        
        $('input[name="total_commission"]').val(commission);

        // calculate total
        var total = 0;
        $('.delivery_products_table tbody tr').each(function() {
            total += parseFloat($(this).find('td:eq(3) p').text());
        });

        // apply commission percentage to total
        var commission_amount = total * commission;
        var total_with_commission = total + commission_amount;

        $('input[name="total"]').val(total_with_commission);

    });

    // filter products by data-lid when selected an option on location_id
    $(document).on('change', 'select[name=location_id]', function() {
        ...
        // each option in select[name=product] has data-lid attribute
        $('select[name=product] option').each(function() {
            if ($(this).data('lid') == location_id) {
        ...
        $('.delivery_products_table tbody tr').remove();
    });

    // delivery product add when clicked on select option
    $(document).on('click', 'button#add_delivery_product', function() {
        var product = $('select[name="product"]').val();
        var productName = $('select[name="product"] option:selected').text();
        var unitPrice = parseFloat($('select[name="product"] option:selected').data('price'));
        var v_id = $('select[name="product"] option:selected').data('vid');
        var quantity = 1;
        var tax = 0;
        var totalPrice = parseFloat(unitPrice * quantity).toFixed(2);

        // check if already exists a row with same product
        var exists = false;
        $('.delivery_products_table tbody tr').each(function() {
            if ($(this).find('input[name="product_id[]"]').val() == product) {
                exists = true;
            }
        ...
        if (exists) {
            ...
            $('.delivery_products_table tbody tr').each(function() {
                if ($(this).find('input[name="product_id[]"]').val() == product) {
                    var oldQuantity = parseInt($(this).find('input[name="quantity[]"]').val());
                    var newQuantity = oldQuantity + 1;
                    $(this).find('input[name="quantity[]"]').val(newQuantity);
                    $(this).find('td:eq(3) p').text(parseFloat(unitPrice * newQuantity).toFixed(2));
                }
            });
            ...
            // apply commission percentage to total
            var commission = $('select[name="user_id"] option:selected').data('commission');
            var commission_amount = total * commission;

            $('input[name="total"]').val(total.toFixed(2));
            $('input[name="commission_amount"]').val(commission_amount.toFixed(2));
        ...
        var newRow = '<tr>' +
            ...
            '<input type="number" name="quantity[]" class="form-control input-sm" value="' + quantity + '" min="1">' +
            '</td>' +
            '<td>' +
            '<input type="number" name="unit_price[]" class="form-control input-sm" value="' + unitPrice.toFixed(2) + '" min="0" step="0.01" readonly>' +
            '</td>' +
            '<td>' +
            '<p>' + totalPrice + '</p>' +
            '</td>' +
        ...
        $('.delivery_products_table').append(newRow);
    ...
    $(document).on('submit', 'form#delivery_group_add_form', function(e) {
        e.preventDefault();
        var data = $(this).serialize();
    ...
    $(document).on('change', 'input[name="quantity[]"]', function() {
        ...
        var unitPrice = $(this).closest('tr').find('input[name="unit_price[]"]').val();
        var totalPrice = parseFloat(quantity * unitPrice);
        $(this).closest('tr').find('td:eq(3) p').text(totalPrice.toFixed(2));
    ...
    /* $(document).on('click', 'button.view_delivery_button', function() {
        $('div.delivery_modal').load($(this).data('href'), function() {
            $(this).modal('show');
        });
    }); */

    $(document).on('click', 'button.add_delivery_return_button', function() {
        $('div.delivery_modal').load($(this).data('href'), function() {
            $(this).modal('show');
        });
    });
    ...
    // remove row if button.delete_product was clicked
    $(document).on('click', 'button.delete_product', function() {
        ...
        // if there are no rows, disable submit button
        if ($('.delivery_products_table tbody tr').length == 0) {
            $('button[type="submit"]').attr('disabled', true);
