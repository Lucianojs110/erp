	...
	<div class="row">
		<div class="@if(!empty($pos_settings['hide_product_suggestion']) && !empty($pos_settings['hide_recent_trans'])) col-md-10 col-md-offset-1 @else col-md-7 @endif col-sm-12">
			<div class="box box-success">
				...
				{{-- has_delivery hidden input --}}
				{!! Form::hidden('has_delivery', $has_delivery, ['id' => 'has_delivery']); !!}
				...
				<div class="box-body">
						...
						<div class="@if(!empty($commission_agent)) col-sm-4 @else col-sm-6 @endif">
							<div class="form-group">
								<div class="input-group">
									...
									@else
										{!! Form::text('search_product', null, ['class' => 'form-control mousetrap', 'id' => 'search_product', 'placeholder' => __('lang_v1.search_product_placeholder'), 'autofocus', 'disabled']); !!}
									@endif
									<span class="input-group-btn">
										<button @if($has_delivery) disabled @endif type="button" class="btn btn-default bg-white btn-flat pos_add_quick_product" data-href="{{action('ProductController@quickAdd')}}" data-container=".quick_add_product_modal"><i class="fa fa-plus-circle text-primary fa-lg"></i></button>
									</span>
								</div>
							</div>
						</div>
					...
					<div class="row col-sm-12 pos_product_div" id="pos_product_list">
						...
						<table class="table table-condensed table-bordered table-striped table-responsive" id="pos_table">
							...
							<tbody>
								@foreach($sell_details as $sell_line)
									@include('sale_pos.product_row', ['has_delivery' => $has_delivery,'product' => $sell_line, 'row_count' => $loop->index, 'tax_dropdown' => $taxes, 'sub_units' => !empty($sell_line->unit_details) ? $sell_line->unit_details : []  ])
								@endforeach
							</tbody>
						</table>
					</div>
					@include('sale_pos.partials.pos_details_edit', ['edit' => true])
					...
					@endif
					@include('sale_pos.partials.payment_express_modal')
				</div>
				<!-- /.box-body -->
				{!! Form::close() !!}
			</div>
		...
		<div class="col-md-5 col-sm-12">
			@include('sale_pos.partials.right_div', ['has_delivery' => $has_delivery])
		</div>
	</div>
</section>
