<tr class="product_row" data-row_index="{{$row_count}}" >
	<td>
		@php
			if (empty($product->tax_id)) {
   				$tax = 'Sin impuesto';
			}elseif ($product->tax_id == 1) {
				$tax = 'IVA 21%';
			}elseif ($product->tax_id == 2) {
				$tax= 'IVA 10,5';
			}elseif ($product->tax_id == 3) {
				$tax = 'IVA + Tasa provincial';
			}elseif ($product->tax_id == 4) {
				$tax = 'IIBB (3.3%)';
			}elseif ($product->tax_id == 5) {
				$tax = 'IVA + IIBB';
			}
			$product_name = $product->product_name . '<br/>' . $tax ;
	...
	<td>
		...
		<input type="text" data-min="0" class="form-control pos_quantity input_number mousetrap input_quantity" value="{{@format_quantity($product->quantity_ordered)}}" name="products[{{$row_count}}][quantity]"   data-allow-overselling="@if(empty($pos_settings['allow_overselling'])){{'false'}}@else{{'true'}}@endif" 
		...
		<span class="input-group-btn"><button type="button" class="btn btn-default btn-flat quantity-up" @if(isset($has_delivery) && $has_delivery) disabled @endif><i class="fa fa-plus text-success"></i></button></span>
		</div>
	...
	<td>
		...
		<input  type="text" readonly name="products[{{$row_count}}][unit_price]" class="form-control pos_unit_price input_number mousetrap" value="{{@num_format($product->sell_price_inc_tax)}}">
		
		<input  type="hidden" value="{{$product->hasMayorista}}" name="products[{{$row_count}}][hasMayorista]" class="form-control hasMayorista" />
		<input  type="hidden" value="{{$product->precioMayorista}}" name="products[{{$row_count}}][precioMayorista]" class="form-control precioMayorista" />
		<input  type="hidden" value="{{$product->cantidadMayorista}}" name="products[{{$row_count}}][cantidadMayorista]" class="form-control cantidadMayorista" />
		<input  type="hidden" name="products[{{$row_count}}][originalPrice]" class="form-control originalPrice" value="{{@num_format($product->sell_price_inc_tax)}}">
	...
	<td class="text-center">
		...
		<input type="{{$subtotal_type}}" name="products[{{$row_count}}][pos_line_total]" readonly class="form-control pos_line_total  input_number" value="{{@num_format($product->quantity_ordered*$product->sell_price_inc_tax)}}">
