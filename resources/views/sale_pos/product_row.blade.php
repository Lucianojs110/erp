<tr class="product_row" data-row_index="{{ $row_count }}">
    <td>
        ...
        @if (auth()->user()->can('edit_product_price_from_sale_screen') ||
                auth()->user()->can('edit_product_discount_from_sale_screen'))
...
@endif
  -->
    ...
    <td>
        ...
        @php
            $manages_packages = (int) ($product->manages_packages ?? 0) === 1;
            $weight_per_meter = (float) ($product->weight ?? 0);
            $package_length = (float) ($product->length ?? 0);
    ...
    <td>
        ...
        <input type="text" readonly name="products[{{ $row_count }}][unit_price]"
            class="form-control pos_unit_price input_number mousetrap"
            value="{{ @num_format($product->sell_price_inc_tax) }}">

        <input type="hidden" value="{{ $product->hasMayorista }}"
            name="products[{{ $row_count }}][hasMayorista]" class="form-control hasMayorista" />
        <input type="hidden" value="{{ $product->precioMayorista }}"
            name="products[{{ $row_count }}][precioMayorista]" class="form-control precioMayorista" />
        <input type="hidden" value="{{ $product->cantidadMayorista }}"
            name="products[{{ $row_count }}][cantidadMayorista]" class="form-control cantidadMayorista" />
        <input type="hidden" name="products[{{ $row_count }}][originalPrice]" class="form-control originalPrice"
            value="{{ @num_format($product->sell_price_inc_tax) }}">


        <input readonly type="hidden" name="products[{{ $row_count }}][tax_id]"
            class="form-control tax_id input_number mousetrap" value="{{ $product->tax_id }}">
        <input readonly type="hidden" name="products[{{ $row_count }}][tax_amount]"
            class="form-control pos_unit_price_neto input_number mousetrap"
    ...
    <td class="{{ $hide_tax }}">
        <input type="text" name="products[{{ $row_count }}][unit_price_inc_tax]"
            ...
            @if (!auth()->user()->can('edit_product_price_from_sale_screen')) readonly @endif
    ...
    <td class="text-center">
        ...
        <input type="{{ $subtotal_type }}" name="products[{{ $row_count }}][pos_line_total]" readonly
            class="form-control pos_line_total  input_number"
            value="{{ @num_format($product->quantity_ordered * $product->sell_price_inc_tax) }}">
