        ...
        <div class="modal-body">
            ...
            <div class="row">
                <div class="col-sm-4">
                    ...
                    @endif
                    <br>
                    <b>{{ __('sale.payment_status') }}:</b> {{ ucfirst($sell->payment_status) }}
                    <br>
                </div>
                <div class="col-sm-4">
                    <b>{{ __('sale.customer_name') }}:</b> {{ $sell->contact->name }}<br>
                    <b>Factura AFIP N°: </b>{{ $sell->num_invoice_afip }}<br>
                    <b>CAE: </b>{{ $sell->cae }}<br>
                    <b>Vto: </b>{{ $sell->exp_cae }}<br>
                ...
                <div class="col-sm-4">
                    <b>{{ __('lang_v1.carries_a_bag') }}:
                    </b>{{ $pos_settings['carries_a_bag'] == 1 ? __('lang_v1.afirmacion') : __('lang_v1.negacion') }}<br>
            ...
            <div class="row">
                        ...
                        <table class="table bg-gray">
                            ...
                            @forelse ($sell->payment_lines as $payment_line)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        {{-- Muestra el metodo de pago en el modal --}}
                                        @if ($payment_line->is_return == 1)
                                            {{ ‘Efectivo’ }}
                                        @elseif($payment_line->is_return == 2)
                                            {{ ‘Tarjeta’ }}
                                        @else
                                            {{ $payment_types[$payment_line->method] ?? $payment_line->method }}
                                        @endif
                                        @if ($payment_line->method === 'cheque' || $payment_line->method === 'Cheques')
                                            <br>
                                            <strong>N° de cheque: {{ $payment_line->cheque_number }} <br>
                                                Monto del cheque: ${{ $payment_line->cheque_amount }}</strong>
                                        @endif
                ...
                <div class="col-md-6 col-sm-12 col-xs-12">
                    <div class="table-responsive">
                        @php
                            $total_payable = $sell->final_total;
                            if (!empty($payment_line)) {
                                $cheque_amount = $payment_line->cheque_amount ?? 0;
                                $total_paid =
                                    $payment_line->method !== 'cheque'
                                        ? $total_payable
                                        : min($cheque_amount, $total_payable);
                            }
                        ...
                        <table class="table bg-gray">
                            ...
                            <tr>
                                ...
                                <td></td>
                                <td><span class="display_currency pull-right"
                                        data-currency_symbol="true">{{ $total_payable }}</span></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td></td>
                                <td><strong><span class="display_currency pull-right text-red"
                                            data-currency_symbol="true">{{ $total_remaining }}</span></strong></td>
                            </tr>
                        </table>
                    </div>
                </div>
