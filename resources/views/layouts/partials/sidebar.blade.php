  ...
  <aside class="main-sidebar">
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu">
        ...
        @endif
        @if(auth()->user()->can('delivery.view') || auth()->user()->can('return.view'))
        <li class="treeview {{ in_array($request->segment(1), ['roles', 'users', 'sales-commission-agents']) ? 'active active-sub' : '' }}">
            <a href="#">
                <i class="fa fa-handshake-o" ></i>
                <span class="title" >@lang('lang_v1.deliveries')</span>
                <span class="pull-right-container">
                    <i class="fa fa-angle-left pull-right"></i>
                </span>
            </a>
            <ul class="treeview-menu">
              ...
              @can( 'delivery.view' )
                ...
                <!-- @can( 'return.view' )
                <li class="{{ $request->segment(1) == 'users' ? 'active active-sub' : '' }}">
                  <a href="{{action('AgentTemporalStockController@index')}}">
        ...
        <li class="treeview @if( in_array($request->segment(1), ['business', 'tax-rates', 'barcodes', 'invoice-schemes', 'business-location', 'invoice-layouts', 'printers', 'subscription']) || in_array($request->segment(2), ['tables', 'modifiers']) ) {{'active active-sub'}} @endif">
          ...
          <ul class="treeview-menu" id="tour_step3">
            ...
            <li class="{{ $request->segment(1) == 'barcodes' ? 'active' : '' }}">
              <a href="{{action('BarcodeController@index')}}"><i class="fa fa-barcode"></i> <span>@lang('barcode.barcodes')</span></a>
            </li>
            @endcan
        ...
        <li class="treeview {{  $request->segment(1) == 'terms-and-conditions' ? 'active active-sub' : '' }}">
            <a href="{{action('HomeController@termsConditions')}}"><i class="fa fa-file-text"></i> <span>Terminos y condiciones</span>
            </a>
        </li>
