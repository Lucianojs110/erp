@foreach( $variations as $variation)
    <tr>
        ...
        <td class="{{$hide_tax}}">
            ...
            <input type="hidden" name="purchases[{{ $row_count }}][row_tax_amount]" class="row_tax_amount_hidden" value="{{ isset($product->product_tax->amount) ? $product->product_tax->amount : 0 }}">
        ...
        @endif
        @if( $product->hasMayorista == '1' )
        <td >
            {!! Form::text('purchases[' . $row_count . '][precioMayorista]', number_format($variation->precioMayorista, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm input_number precioMayorista', 'required']); !!}
        </td>
        @else
        <td >
        {!! Form::text('purchases[' . $row_count . '][precioMayorista]', number_format($variation->precioMayorista, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm input_number precioMayorista', 'disabled']); !!}
        </td>
        @endif
        <?php $row_count++ ;?>
