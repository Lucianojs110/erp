    ...
    <div class="modal-body">
      <div class="row">
        ...
        @elseif(in_array($due_payment_type, ['sell']))
          <div class="col-md-6">
            <div class="well">
              <strong>@lang('sale.customer_name'): </strong>{{ $contact_details->name }}<br><br>
              <!-- button to see customer transactions -->
              <a href="{{action('ContactController@show', ['contact' => $contact_details->contact_id, 'payment_status' => 'due'])}}" class="btn btn-danger btn-xs">
                <i class="fa fa-eye"></i> @lang('lang_v1.sell_due')
              </a>
              <br><br>
            </div>
          </div>
          <div class="col-md-6">
            <div class="well">
              ...
              <strong>@lang('contact.total_paid'): </strong><span class="display_currency" data-currency_symbol="true">{{ $contact_details->total_paid }}</span><br>
              <strong>@lang('contact.total_sale_due'): </strong><span class="display_currency text-red text-bold" data-currency_symbol="true">{{ $contact_details->total_invoice - $contact_details->total_paid }}</span><br>
              @if(!empty($contact_details->opening_balance) || $contact_details->opening_balance != '0.00')
                  <strong>@lang('lang_v1.opening_balance'): </strong>
                  <span class="display_currency" data-currency_symbol="true">
                  {{ $contact_details->opening_balance }}</span><br>
