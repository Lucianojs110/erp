...
{!! Form::hidden('location_id', $sell->location->id, ['id' => 'location_id', 'data-receipt_printer_type' => $sell->location->receipt_printer_type ]); !!}
	...
	<div class="box box-solid">
		<div class="box-body">
				        ...
				        <tbody>
				            ...
				            <tr>
				              	...
				              	<td>
									...
									<input name="products[{{$loop->index}}][tax_id]" type="hidden" value="{{$sell_line->tax_id}}">
				              	</td>
				              	<td>
			...
			<h4>Montos Devueltos (Para registro de caja):</h4>
			<div class="row">
			...
			@foreach($sell->payment_lines as $payment)
					...
					<div class="form-group">
						<label>@lang('lang_v1.'.$payment->method)</label>
						<input  name="payment[{{$loop->index}}][method]" type="hidden" value="{{$payment->method}}">
						<input name="payment[{{$loop->index}}][amount]" class="form-control" type="number" value="{{$payment->amount}}">
					</div>
